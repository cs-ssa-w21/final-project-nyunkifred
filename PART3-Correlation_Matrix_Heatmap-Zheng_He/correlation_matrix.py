...
def append_us_covid_growth_data():
    ...
    Return:
        ...
        and their frequencies; 4 independent variables (parameters) and their
        frequencies: us_covid_positive -- total number of positive covid-19 
        cases in each quarter of 2020; us_covid_positive_case_growth rate % -- 
        quarterly growth rate of positive covid-19 cases; us_covid_death -- 
        total number of covid-19 deaths in each quarter of 2020; 
        us_covid_death_growth rate % -- quarterly growth rate of covid-19 deaths.
    '''
    ...
    us_covid_positive_row = pd.DataFrame([["us covid positive", qt_1_positive,\
                                            qt_2_positive, qt_3_positive,\
                                            qt_4_positive]])
    us_covid_positive_row.columns = ['keyword', '2020_qt1', '2020_qt2',\
                                      '2020_qt3', '2020_qt4']
    
    us_covid_death_row = pd.DataFrame([["us covid death", qt_1_death,\
                                            qt_2_death, qt_3_death,\
                                            qt_4_death]])
    us_covid_death_row.columns = ['keyword', '2020_qt1', '2020_qt2',\
                                      '2020_qt3', '2020_qt4']
    
    us_covid_positive_growth_row = pd.DataFrame([["us covid positive case growth rate %",\
    ...
    us_covid_death_growth_row = pd.DataFrame([["us covid death growth rate %",\
...
def append_us_unemployment_rate():
    '''
    Append "the US unemployment rate" parameter to the dataframe. Here we used
    the OECD data (Link: https://data.oecd.org/unemp/unemployment-rate.htm ) 
    where the quarterly unemployment rates of the U.S. are provided in a 
    ...
    us_unemployent_row = pd.DataFrame([["us unemployment rate %", OECD_csv[6][1],\
...
def append_us_productivity():
    ...
    us_prod_row = pd.DataFrame([["us productivity %", prod_qt_1, prod_qt_2,\
                                 prod_qt_3, prod_qt_4]])
